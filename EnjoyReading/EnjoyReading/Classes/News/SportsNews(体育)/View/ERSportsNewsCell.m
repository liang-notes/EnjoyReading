...
@implementation ERSportsNewsCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    ...
    ERSportsNewsCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[ERSportsNewsCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
- (void)setSportsNews:(ERSportsNews *)sportsNews
{
    _sportsNews = sportsNews;
    
    [self setupFrames];
    
    self.titleLabel.text = sportsNews.title;
    
    if (sportsNews.picUrl.length) {
        self.pictureView.hidden = NO;
        [self.pictureView sd_setImageWithURL:[NSURL URLWithString:sportsNews.picUrl]];
    }else{
        self.pictureView.hidden = YES;
    }
    self.ctimeLabel.text = sportsNews.ctime;
    self.erdescriptionLabel.text = sportsNews.erdescription;
...
- (void)setupFrames
{
    ...
    CGSize titleLabelSize = [_sportsNews.title boundingRectWithSize:CGSizeMake(titleLabelW, MAXFLOAT) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:NewsTitleFont} context:nil].size;
    self.titleLabel.frame = (CGRect){{titleLabelX,titleLabelY},titleLabelSize};
    
    if (self.sportsNews.picUrl.length){
        // 图片
        ...
        CGSize erdescriptionLabelSize = [_sportsNews.erdescription sizeWithAttributes:@{NSFontAttributeName:NewsErdescriptionFont}];
        ...
        CGSize ctimeLabelSize = [_sportsNews.ctime sizeWithAttributes:@{NSFontAttributeName:NewsCtimeFont}];
        ...
        self.sportsNews.cellHeight = CGRectGetMaxY(self.spView.frame);
